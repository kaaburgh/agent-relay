...
class CancelLeaseTests(unittest.TestCase):
    ...
    def test_cancel_retains_ssh_attempt_lease_and_reports_reason(self) -> None:
        async def scenario() -> None:
            ...
            self.assertEqual(
                result["retained_leases"],
                [{
                    "lease_id": lease.lease_id,
                    "resource": "runtime",
                    "attempt_id": attempt.attempt_id,
                    "reason": "remote-ssh-transport",
                }],
            )
            remaining = active_leases(self.store, "runtime")
            self.assertEqual(len(remaining), 1)
            self.assertEqual(remaining[0].lease_id, lease.lease_id)
            self.assertEqual(result["status"]["leases"][0]["lease_id"], lease.lease_id)
            await managed.wait()
